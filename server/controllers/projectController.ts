...
export const makeRevision = async(req:Request, res:Response)=>{
    ...
    try{
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: `I've enhacned your prompt to "${enhancedPrompt}"`,
                projectId: String(projectId),
            }
        })
        ...
        const codeGenerationResposne = await openai.chat.completions.create({
            model: 'z-ai/glm-4.5-air:free',
            messages: [
                {
                    role: 'system',
        ...
        const code = codeGenerationResposne.choices[0].message.content || '';
        ...
        res.json({message: 'Changes made succesfully'})
    }
    catch (error: any){
        await prisma.user.update({
            where: {id: userId},
...
export const rollbackToVersion = async(req:Request, res:Response)=>{
    try{
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: `I've rolled back to slected version. You can now preview it.`,
                projectId: String(projectId),
            }
        })
    } catch (error: any) {
